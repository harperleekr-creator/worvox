// Send announcement email to ALL users via Resend API
...
// Get all user emails from API endpoint
async function getAllUserEmails() {
  try {
    const response = await fetch('https://worvox.com/api/cron/get-user-emails', {
      headers: {
        'Authorization': 'Bearer worvox-cron-secret-2024-secure-key'
      }
    });
...
async function sendAnnouncementToAll() {
  console.log('📧 Starting mass announcement email sending...\n');
  ...
  try {
    ...
    for (let i = 0; i < emails.length; i++) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          failCount++;
          console.error(`❌ [${i + 1}/${emails.length}] Failed to: ${email} - ${result.message}`);
          results.push({ email, status: 'failed', error: result.message });
        }

        // Add delay to avoid rate limiting (200ms between emails)
        ...
        failCount++;
        console.error(`❌ [${i + 1}/${emails.length}] Error sending to ${email}:`, error.message);
    ...
    const fs = require('fs');
    fs.writeFileSync(
      'announcement-results.json',
      JSON.stringify({ timestamp: new Date().toISOString(), results }, null, 2)
    );
    console.log('\n💾 Results saved to announcement-results.json');
