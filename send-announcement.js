...
async function sendAnnouncement() {
  console.log('📧 Starting announcement email sending to all users...\n');

  try {
    // Import required module for D1 database access
    const { execSync } = require('child_process');
    
    // Fetch all users with email from database
    const usersQuery = `SELECT email FROM users WHERE email IS NOT NULL AND email != ''`;
    const result = execSync(`cd /home/user/webapp && npx wrangler d1 execute worvox-production --remote --command="${usersQuery}" --json`, {
      encoding: 'utf-8'
    });
    ...
    for (const user of users) {
      try {
        ...
        if (response.ok) {
          console.log(`✅ Sent to: ${user.email} (ID: ${responseData.id})`);
          successCount++;
        } else {
          console.error(`❌ Failed to send to ${user.email}:`, responseData);
          failCount++;
        }
        
        // Rate limiting: wait 300ms between emails (5 req/sec = 200ms minimum)
