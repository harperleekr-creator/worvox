...
class WorVox {
    ...
    if (transcription && transcription !== '(인식되지 않음)' && this.currentUser?.plan === 'premium') {
      ...
      setTimeout(async () => {
        try {
          ...
          console.log(`⏱️ AI Analysis completed in ${aiEndTime - aiStartTime}ms (Backend: ${analysisResponse.data.processingTime || 'N/A'}ms)`);
  ...
  async processExamAnswer(audioBlob) {
    ...
    try {
      ...
      if (transcription) {
        try {
          const examAnalysisStartTime = Date.now();
          ...
          const examAnalysisEndTime = Date.now();
          console.log(`⏱️ Exam analysis completed in ${examAnalysisEndTime - examAnalysisStartTime}ms (Backend: ${analysisResponse.data.processingTime || 'N/A'}ms, Cached: ${analysisResponse.data.cached ? 'YES' : 'NO'})`);
